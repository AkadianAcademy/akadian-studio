    ...
    <div style={{ maxWidth: '680px', width: '100%' }}>
      ...
      <div style={{ marginBottom: '36px' }}>
        ...
        <h1 style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 700, color: '#1A1219', letterSpacing: '-0.025em', lineHeight: 1.1, marginBottom: '12px' }}>
          Words that <span style={{ color: '#7B5CFF', fontStyle: 'italic' }}>open doors</span>
        </h1>
        <p style={{ fontSize: '15px', color: '#6B6575', lineHeight: '1.6', maxWidth: '520px' }}>
      ...
      <div style={{ marginBottom: '40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#1A1219', marginBottom: '2px' }}>Vocabulary</h2>
            <p style={{ fontSize: '12px', color: '#9090A0' }}>Core words for the lesson</p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px' }}>
          {vocab.length === 0 ? (
            <div style={{ padding: '24px', textAlign: 'center', background: '#FAFAF8', border: '1px dashed rgba(255,255,255,0.08)', borderRadius: '12px' }}>
              <p style={{ fontSize: '13px', color: '#B0A8C0' }}>
                No vocabulary yet — click <span style={{ color: '#7B5CFF' }}>AI suggest vocabulary</span> or add words manually
              </p>
            </div>
          ) : (
            vocab.map((v, i) => (
        ...
        <button onClick={addVocabRow} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', background: 'transparent', border: '1px dashed rgba(255,255,255,0.15)', borderRadius: '10px', color: '#6B6575', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
          + Add word
        </button>

        {suggestVocabAI.error && (
          <p style={{ fontSize: '12px', color: '#7B5CFF', marginTop: '8px' }}>⚠ {suggestVocabAI.error}</p>
        )}
      </div>
      ...
      <div style={{ marginBottom: '40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#1A1219', marginBottom: '2px' }}>Example sentences</h2>
            <p style={{ fontSize: '12px', color: '#9090A0' }}>Show how the language works in context</p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '12px' }}>
          {sentences.length === 0 ? (
            <div style={{ padding: '24px', textAlign: 'center', background: '#FAFAF8', border: '1px dashed rgba(255,255,255,0.08)', borderRadius: '12px' }}>
              <p style={{ fontSize: '13px', color: '#B0A8C0' }}>
                No sentences yet — click <span style={{ color: '#7B5CFF' }}>AI add examples</span> above or add manually
              </p>
            </div>
          ) : (
            sentences.map((s, i) => (
        ...
        <button onClick={addSentenceRow} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', background: 'transparent', border: '1px dashed rgba(255,255,255,0.15)', borderRadius: '10px', color: '#6B6575', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
          + Add sentence
        </button>

        {addExamplesAI.error && (
          <p style={{ fontSize: '12px', color: '#7B5CFF', marginTop: '8px' }}>⚠ {addExamplesAI.error}</p>
        )}
      </div>

      {/* Navigation */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        <button onClick={onBack} style={{ padding: '13px 24px', background: 'transparent', border: '1px solid rgba(91,60,224,0.12)', borderRadius: '12px', color: '#6B6575', fontSize: '14px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
          ← Back
        </button>
        <button
